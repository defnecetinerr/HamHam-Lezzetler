...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        let rememberUser = UserDefaults.standard.bool(forKey: "RememberUser")
              ...
              if rememberUser, let currentUser = Auth.auth().currentUser {
                  // Oturum açık ve "Beni Hatırla" işaretli ise
                  showMainPage()
              } else {
                  let isFirstLaunch = UserDefaults.standard.bool(forKey: "isFirstLaunch")
                  
                  if isFirstLaunch {
                      // İlk kez başlatılan uygulama
                      showOnboarding()
                  } else {
                      // Oturum açma ekranını göster
                      showSignIn()
                  }
              }
    ...
    func showMainPage() {
     ...
     func showSignIn() {
         let storyboard = UIStoryboard(name: "Main", bundle: nil)
         let signInVC = storyboard.instantiateViewController(withIdentifier: "loginVC") as! SignViewController
    ...
    lazy var persistentContainer: NSPersistentContainer = {
        ...
        */
        let container = NSPersistentContainer(name: "HamHam_Lezzetler")
